...
| Open-Meteo | Viewed-location weather chip/panel and optional cloud/precip forecast overlay | No key, CORS `*`, hidden below zoom 5, 15-minute cache, coordinates rounded to 2 decimals, 1 s `moveend` debounce, skip unchanged cells, 5×5 overlay samples, 2 forecast days | Free non-commercial API; attribute Open-Meteo. Overlay is a coarse model grid, not radar. Replace or proxy before ads, subscriptions, or high traffic. |
