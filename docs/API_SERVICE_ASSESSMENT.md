...
| Digitraffic road weather and TMS | Finnish road weather and traffic colouring | `Digitraffic-User` header, 30-minute station cache, 90-second observation cache, 2-minute refresh while the layer is visible | Public road API; gzipped bulk payloads are fetched only after the layer is enabled. |
