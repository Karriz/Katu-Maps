...
function applyPalette(map: MapLibreMap, colors: DayNightPalette, night: number) {
  ...
  ['global-building-footprints', 'global-building-footprints-2d'].forEach((id) => {
    setPaint(map, id, 'fill-color', buildingColorExpression(colors.building));
    setPaint(map, id, 'fill-outline-color', colors.boundary);
  });
  setPaint(map, 'global-building-ground-storeys', 'fill-extrusion-color', buildingColorExpression(colors.buildingBand));
  setPaint(map, 'global-buildings', 'fill-extrusion-color', buildingColorExpression(colors.building));
