...
export function createGroundPattern(size: number, kind: GroundPatternKind, theme: 'light' | 'dark') {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      let coverage = 0;
      if (kind === 'farmland') {
        const stripe = (localX + localY * 0.45 + hash(cellX, cellY, 4) * 0.28) % 1;
        coverage = stripe > 0.44 && stripe < 0.56 ? 0.65 : 0;
      } else if (kind === 'wetland') {
        const ripple = (localY + hash(cellX, cellY, 4) * 0.25) % 0.5;
        coverage = ripple > 0.18 && ripple < 0.3 ? 0.58 : 0;
      } else if (kind === 'sand' || kind === 'rock') {
        coverage = hash(cellX * 3 + Math.floor(localX * 3), cellY * 3 + Math.floor(localY * 3), kind === 'sand' ? 9 : 10) > 0.72 ? 0.38 : 0;
      } else if (kind === 'forest' || kind === 'scrub') {
        const dx = localX - (0.25 + hash(cellX, cellY, 5) * 0.5);
        const dy = localY - (0.25 + hash(cellX, cellY, 6) * 0.5);
        ...
        const firstX = 0.16 + hash(cellX, cellY, 7) * 0.68;
        const firstY = 0.16 + hash(cellX, cellY, 8) * 0.68;
        const secondX = 0.16 + hash(cellX, cellY, 11) * 0.68;
        const secondY = 0.16 + hash(cellX, cellY, 12) * 0.68;
...
export function groundPatternLayers(): FillLayerSpecification[] {
  ...
  return layers.map((layer, index) => ({
    ...layer,
    paint: {
      'fill-pattern': groundPatternImageId(PATTERN_KINDS[index], 'light'),
...
export function applyGroundPatternTheme(map: { getLayer: (id: string) => unknown; setPaintProperty: (id: string, property: any, value: any) => unknown }, theme: 'light' | 'dark') {
  GROUND_PATTERN_LAYER_IDS.forEach((layerId, index) => {
    if (map.getLayer(layerId)) map.setPaintProperty(layerId, 'fill-pattern', groundPatternImageId(PATTERN_KINDS[index], theme));
  });
}
