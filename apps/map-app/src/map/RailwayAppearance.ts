...
export function railwayWidth(metres: number, gap = false, latitude = 0): ExpressionSpecification {
  const safeLatitude = Math.max(-80, Math.min(80, latitude));
  const pixels = (zoom: number) => metres * 512 * 2 ** zoom
    / (40_075_016.686 * Math.cos(safeLatitude * Math.PI / 180));
