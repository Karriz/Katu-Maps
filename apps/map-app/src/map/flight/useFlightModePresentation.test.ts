...
describe('flight presentation', () => {
  it('hides map POIs, transit, road labels, and application overlays', () => {
    ...
    expect(shouldHideLayerInFlight({ id: 'global-road-labels', type: 'symbol' } as any)).toBe(true);
    expect(shouldHideLayerInFlight({ id: 'global-road-labels-regional', type: 'symbol' } as any)).toBe(true);
  });
